...
namespace HuadianWF
{
    ...
    public class PushInterface : System.Web.Services.WebService
    {
        ...
        //}
        [WebMethod(Description = "向司机推送消息")]
        public string pushToDriver(string number,string title, string content)
        {
            JPushClient client = new JPushClient(app_key_driver, master_secret_driver);
            PushPayload tags = JPushApi.PushObject_android_and_ios(number, title, content, "{\"code\":\"300\"}");
            try
            {
        ...
        [WebMethod(Description = "向货主推送消息")]
        public void pushToClient(string number,string title,string content) {
            JPushApi.push("client-" + number, title, content);
        }
